#include "sys.h"
	...
	u8 res;         //蓝牙接收变量
float Angle_Balance,Gyro_Balance;     //6050的值 
void Bluetooth(void);
...
//	    DMP_Init();                  //6050_DMP初始化
	 ...
	 //电机pwm初始化
		Timer_init(7199,0);   //不分频，pwm频率为72M/7200=10K hz  ，pwm计数总数为7200，定时器3内部输出比较寄存器装载值与7200比较
     	Mortor_init();
	...
	{
//	Read_DMP(); 
	Motor_PWM(3000,3000) ;      //PWM极限为6900.（值越大，越慢）
//	Speed();
//     Olcd_Display();
	}
 }
...
void Speed_Show(u16 left,u16 right,u16 tms)
{
	...
	OLED_ShowNum(40,0,left,9,16);
	
	OLED_ShowString(0,2,"RIGHT",16);
	OLED_ShowChar(40,2,':',16);
	OLED_ShowNum(48,2,right,8,16);
	
//	OLED_ShowString(0,4,"Angle",8);
//	OLED_ShowChar(46,4,':',8);
//	OLED_ShowNum(52,4,Angle_Balance,8,8);
//	
//	OLED_ShowString(0,5,"Gyro",8);
//	OLED_ShowChar(46,5,':',8);
//	OLED_ShowNum(52,5,Gyro_Balance,8,8);
//	delay_ms(tms);
}
void Speed()              //测速并显示
{
	u16 s_left,s_right;
	s_left=Read_val(2);  //左轮
	s_right=Read_val(4);  //右轮
	Speed_Show(s_left,s_right,0);
}
