...
//----------------------按键端口定义-------------------
	...
	enum  KEY_VALUE{KEY_ONE = 0x01,KEY_TWO = 0x02, KEY_THREE = 0x04, KEY_FOUR = 0x08,KEY_FIVE=0x10,KEY_NULL = 0x00};
//------------------------------------------------------------------------------------
    KEY_EXT void  init_key(void);        //按键驱动初始化
    KEY_EXT uchar get_key_value(void);	 //扫描键盘,获得键值
	KEY_EXT void  delay_us(uint us);	 //毫秒级延时函数
#endif
