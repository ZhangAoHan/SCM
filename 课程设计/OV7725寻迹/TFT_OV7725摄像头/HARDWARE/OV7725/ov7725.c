...
void OV7725_Light_Mode(u8 mode)
{
	switch(mode)
	{
		...
		case 1://sunny，晴天
			...
			SCCB_WR_Reg(0x01, 0x5a);   //蓝色通道增益设置 
...
void OV7725_Brightness(s8 bright)
{
	...
	SCCB_WR_Reg(BRIGHT, bright_value);   //亮度
...
void OV7725_Special_Effects(u8 eft)
{
	switch(eft)
	{
		case 0://正常
			...
			SCCB_WR_Reg(0x60, 0x80);//MANV,手动V值    是默认值
			SCCB_WR_Reg(0x61, 0x80);//MANU,手动U值    是默认值
			break;
		case 1://负片
		...
		case 3://偏红
			...
			break;
		case 4://偏绿
			SCCB_WR_Reg(0xa6, 0x1e);  //灰度图，亮度，色度，对比度使能
			SCCB_WR_Reg(0x60, 0x60);   //设置u值
			SCCB_WR_Reg(0x61, 0x60);	 //设置v值	
			break;
		case 5://偏蓝
			SCCB_WR_Reg(0xa6, 0x1e);
...
void OV7725_Window_Set(u16 width,u16 height,u8 mode)
{
	...
	}else
	{
		sx=(320-width)/2;
		sy=(240-height)/2;
		SCCB_WR_Reg(COM7,0x46);		//设置为QVGA模式  数据格式为RGB565
