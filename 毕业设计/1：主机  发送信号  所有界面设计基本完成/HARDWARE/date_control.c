...
const  u8* kbd_delFR[15]={"返回"," : "," ","1","2","3","4","5","6","7","8","9","DEL","0","Enter",};//按键表
extern u8 page;
extern char *str1;
u8 people;
...
void use_as608_scan(void)  //检测指纹函数，将得出用户是否录过指纹。以及是否是来取车的  page=0的运行函数 
{
	if(PS_Sta)	 //检测PS_Sta状态，如果有手指按下
	{
		...
		if(people==1)    //查找成功
		{
			if(people_id==gly_id)//管理员登录    右手拇指
			{
				LCD_Clear(WHITE); 
				delay_ms(50);	
				page=3;   //管理员登录界面
...
u8 id1,id2,id3;
	...
	while(1)
	{
		...
		if(key_num)
		{
			if(key_num==1)Del_FR();		//删指纹  并且禁止删除全部指纹 来保护管理员的指纹数据信息
...
void Gly_page_0_display(void)           //管理员登录成功后进入的主页面  page==3
{
	POINT_COLOR=BLUE;
...
void Gly_page_1_display(void)       //停车位信息  
{
	POINT_COLOR=BLUE;
	...
	LCD_ShowNum(130,75,car_id[0],3,16);      //显示ID
	LCD_ShowNum(130,135,car_id[1],3,16);
	LCD_ShowNum(130,195,car_id[2],3,16);
	LCD_ShowNum(130,255,car_id[3],3,16);
...
void Gly_page_2_display(void)    //用户信息管理
{
	u8 key_num;
	dessce=1;
	POINT_COLOR=BLUE;
