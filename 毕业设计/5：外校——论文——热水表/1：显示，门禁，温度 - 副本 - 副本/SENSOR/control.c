...
void main_display(void)              //主页面  page=0
{
	date_read();
	POINT_COLOR=BLUE;
	Show_Str(24,5,240,24,"水文检测系统",24,0);
	LCD_DrawRectangle(5,30,235,214);
	LCD_DrawRectangle(4,29,236,215);
	POINT_COLOR=RED;
	Show_Str(36,35,168,24,"入温度数据",24,0);
	Show_Str(36,65,168,24,"水浑浊度数据",24,0);
	POINT_COLOR=BLACK;
	Show_Str(10,95,144,24,"水PH值数据",24,0);
...
//	Show_Str(10,155,108,24,"液面情况：",24,0);
	Show_Str(10,155,108,24,"环境温湿度",24,0);
	POINT_COLOR=RED;
	Show_Str(10,185,96,24,"水导电率数据：",24,0);
		POINT_COLOR=BLACK;
	if(tim==0) 
	{
