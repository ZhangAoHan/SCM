...
//vsxset:VS1053的设置
//返回值:0,ok;其他,取消或者错误.
u8 sysset_vs1053_vol3d_set(u16 x,u16 y,_vs10xx_obj *vsxset,u8* caption)
{
 	u8 rval=0,res;
	u8 i=0;
 	_window_obj* twin=0;//窗体
	...
	else
	{
		...
		for(i=0;i<2;i++)
		{
			...
			gui_show_string((u8*)sysset_vs1053vol3d_tbl[i][gui_phy.language],x+10,y+32+12+50*i,190,12,12,SYSSET_INWIN_FONT_COLOR);//显示各項的名字
...
//eqx:0,低音设置;1,高音设置;
//返回值:0,ok;其他,取消或者错误.
u8 sysset_vs1053_eq_set(u16 x,u16 y,_vs10xx_obj *vsxset,u8 eqx,u8* caption)
{
 	u8 rval=0,res;
	u8 i=0;
 	_window_obj* twin=0;//窗体
	...
	else
	{
		...
		for(i=0;i<2;i++)
		{
			...
			vs1053prgb[i]->prgbarlen=5;			//滚条长度设置为5
			progressbar_draw_progressbar(vs1053prgb[i]);	//显示进度条
			gui_show_string((u8*)sysset_vs1053eq_tbl[i][gui_phy.language],x+10,y+32+12+50*i,190,12,12,SYSSET_INWIN_FONT_COLOR);//显示各項的名字
